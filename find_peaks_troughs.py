...
if __name__ == '__main__':
    ...
    parser.add_argument('--PERCENTAGE_CHANGE', required=True, help='Please type in the % chg as an as decimal (i.e. 5 = 5%, 7 = 7%, etc.)')
    parser.add_argument('--INPUT_PATH', required=True, help='Please type a string for the path where your data is located (i.e. "Your_Path/amazonstockprice.xlsx" )')
    parser.add_argument('--OUTPUT_PATH', required=True, help='Please type a string for the path where to output (i.e. "Your_Path/amazonstockprice_output.xlsx" )')
    ...
    for rx in range(sh.nrows):
        py_date = datetime(*xlrd.xldate_as_tuple(sh.row(rx)[0].value, book.datemode))
        ...
        while j < length:
            ...
            if percent_chg > per_chg or percent_chg < -per_chg:
                if results_dict.get(data[i][0].date()):
                    if (percent_chg > per_chg and percent_chg > results_dict[data[i][0].date()][4] and results_dict[data[i][0].date()][4] > 0) or (percent_chg < -per_chg and percent_chg < results_dict[data[i][0].date()][4] and results_dict[data[i][0].date()][4] < 0):
                        # we record a new high or low for this range because we're trending
                        results_dict[data[i][0].date()] = [data[i][0].date(), data[j][0].date(), data[i][1], data[j][1], percent_chg, (data[j][0] - data[i][0]).days, j]
                        ...
                        i = results_dict[data[i][0].date()][6] - 1 # skip to the previous peak/trough, to make sure and record the abrupt move in opposite direction of trend
                        break
                    elif abs(((data[j][1] - results_dict[data[i][0].date()][3])/results_dict[data[i][0].date()][3]) * 100) > per_chg: # chk drawback or bounce > per_chg
                        i = results_dict[data[i][0].date()][6] - 1 # skip to the previous peak
                        break
                else:  # this is the case in which it's not in the results_dict meaning have not visited price
                    results_dict[data[i][0].date()] = [data[i][0].date(), data[j][0].date(), data[i][1], data[j][1], percent_chg, (data[j][0] - data[i][0]).days, j]
            elif results_dict.get(data[i][0].date()): # case looking for drawbacks that percentage change doesn't meet the per_chg% mark
                if abs(((data[j][1] - results_dict[data[i][0].date()][3])/results_dict[data[i][0].date()][3]) * 100) > per_chg: # result in which we draw back per_chg%
                    i = results_dict[data[i][0].date()][6] - 1 # skip to the previous peak/trough
                    break
            j += 1
        if end_leg == True:
            break
    ...
    for key in sorted(results_dict.iterkeys()):
        ...
        for element in results_dict[key]:
            if col < 2:
                worksheet.write_datetime(row, col, element, date_format) #putting in headers for sheet
            else:
